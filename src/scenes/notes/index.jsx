...
const Notes = () => {
  ...
  try {
    ...
    const res = await axios.get(`http://161.35.54.196/api/v1/noteGetByid/${createrid}`);
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await axios.post("http://161.35.54.196/api/v1/createNote", payload, {
...
const handleEditSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.put(`http://161.35.54.196/api/v1/UpdateNoteDetails/${editNote.id}`, payload, {
        headers: { "Content-Type": "application/json" },
      });
      if (res.status === 200) {
  ...
  const handleDeleteNote = async () => {
    try {
      setIsLoading(true);
      const res = await axios.delete(`http://161.35.54.196/api/v1/NoteDelete/${editNote.id}`);
      if (res.status === 200) {

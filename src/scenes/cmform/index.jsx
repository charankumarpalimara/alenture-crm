...
import { Form, Input, Button, Select, Row, Col, Avatar, Modal, Typography, message, Spin } from "antd";
...
const handleFormSubmit = async (values) => {
  ...
  try {
    ...
    setIsLoading(false);
    navigate("/cm"); // use the hook, not Navigate("/cm")
  } catch (error) {
...
const uniqueBranchNames = Array.from(new Set(branchNames));
            ...
            <Col>
              <div style={{ position: 'relative', display: 'inline-block' }}>
                <Avatar
                  ...
                  size={120}
                  style={{ border: '2px solid #1677ff', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}
                  onClick={triggerFileInput}
                />
                <Button
                  icon={<CameraOutlined />}
                  shape="circle"
                  style={{ position: 'absolute', bottom: 0, right: 0, background: '#1677ff', color: '#fff', border: 'none' }}
                  onClick={triggerFileInput}
                />
                <input
                  type="file"
  ...
  >
    <Input placeholder="First Name" size="large" style={{ borderRadius: 8, background: '#fff', fontSize: 16 }} />
  </Form.Item>
</Col>
<Col xs={24} md={8}>
  <Form.Item
  ...
  >
    <Input placeholder="Last Name" size="large" style={{ borderRadius: 8, background: '#fff', fontSize: 16 }} />
  </Form.Item>
</Col>
<Col xs={24} md={8}>
  <Form.Item
  ...
  >
    <Input placeholder="Email" size="large" style={{ borderRadius: 8, background: '#fff', fontSize: 16 }} />
  </Form.Item>
</Col>
<Col xs={24} md={8}>
  <Form.Item
  ...
  >
    <Input.Group compact>
      ...
      >
        ...
        >
  {countries.map((c) => (
  <Select.Option key={`${c.isoCode}-${c.phonecode}`} value={`+${c.phonecode}`}>{`+${c.phonecode} (${c.name})`}</Select.Option>
))}
        </Select>
      </Form.Item>
      <Form.Item
      ...
      >
        <Input style={{ width: 'calc(100% - 160px)' }} placeholder="Phone Number" size="large" />
      </Form.Item>
    </Input.Group>
  </Form.Item>
</Col>
...
<Col xs={24} md={8}>
  ...
  >
    <Input placeholder="Designation" size="large" style={{ borderRadius: 8, background: '#fff', fontSize: 16 }} />
  </Form.Item>
</Col>
<Col xs={24} md={8}>
  <Form.Item
  ...
  >
    <Select
      ...
      size="large"
      style={{ borderRadius: 8, background: '#fff', fontSize: 16 }}
      onChange={async (value) => { form.setFieldsValue({ organization: value, branch: '' }); await fetchBranch(value); }}
    >
      {organizationNames.map((org) => (<Option key={org} value={org}>{org}</Option>))}
    </Select>
  </Form.Item>
</Col>
<Col xs={24} md={8}>
      ...
      {uniqueBranchNames.map((b) => (
        <Option key={b} value={b}>{b}</Option>
          ...
          <Row justify="end" style={{ marginTop: 32 }} gutter={16}>
            <Col>
              <Button type="primary" htmlType="submit" size="large" style={{ background: '#3e4396', color: '#fff', fontWeight: 'bold', borderRadius: 8 }}>Create</Button>
            </Col>
          </Row>
        </Form>
      </div>
